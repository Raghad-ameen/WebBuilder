...
export default function SectionRenderer({ section, selectedElementIds = [], onSelect, store, canvasScale = 1 }) {
  ...
  // Compute canvas height from section height or default
  const canvasHeight = section.height || 500;
  ...
  const handleSubmitForm = async (sectionId, action, isPopup = false, clickedItemId = null) => {
    ...
    if (hasSpecificBindings) {
      ...
      formFields = sectionItems.filter(item => {
        ...
        if (finalIsPopup) return item.type === "input";
    ...
    try {
      ...
      console.error("❌ فشل الإرسال للباكيند:", error.response?.data || error.message);
    }
  };
  ...
  const innerContainerStyle = state.isPreviewMode ? {
    width: "1200px",
    minWidth: "1200px",
    position: "relative",
    height: "100%",
  ...
  return (
    <div
      ...
      onMouseDown={(e) => {
        ...
        if (e.target.classList.contains('selecto-area') || e.target.closest('.selecto-area')) return;
      ...
      onMouseUp={(e) => {
        ...
        if (sectionTypes.includes(state.draggingType)) {
          if (store.addSection) {
            store.addSection(state.draggingType, []);
          }
        ...
        const defaultWidth  = (state.draggingType === 'input' || state.draggingType === 'button') ? 250 : 150;
        const defaultHeight = (state.draggingType === 'input' || state.draggingType === 'button') ? 45  : 100;

        // Clamp starting position to canvas bounds
        const clampedX = Math.min(Math.max(0, x - defaultWidth / 2), CANVAS_WIDTH - defaultWidth);
        const clampedY = Math.max(0, y - defaultHeight / 2);
    ...
    >
      ...
      {isSectionSelected && !state.isPreviewMode && !section.isGhost && (
        <div style={styles.sectionToolbar}>
          <button
            onPointerDown={(e) => { e.stopPropagation(); deleteSection(section.id); }}
      ...
      <div style={
        state.isPreviewMode
          ? { width: "100%", display: "flex", justifyContent: "center", position: "relative", height: "100%", overflow: "visible" }
        ...
        <div style={innerContainerStyle}>
            ...
            return (
              <React.Fragment key={item.id}>
                ...
                <div
                  ...
                  onMouseDown={(e) => {
                    ...
                    if (isCtrl) {
                      ...
                      const newSelection = currentSelected.includes(targetId)
                  ...
                  onMouseUp={() => {
                    if (!state.isPreviewMode) setIsSelecting(false);
                  }}
                  style={{
                    position: "absolute",
                    left: `${item.x}px`,
                ...
                >
                  ...
                  {/* Element renderers */}
                  {item.type === "text" && (
                    <TextElement item={item} state={state} store={store} section={section} isMobile={isMobile} cleanFilter={cleanFilter} />
                  )}
                  {item.type === "image" && (
                    <ImageElement item={item} state={state} isSelected={isSelected} cleanFilter={cleanFilter} updateItem={updateItem} activePageId={activePageId} section={section} />
                  )}
                  {item.type === "shape" && (
                    <ShapeElement item={item} state={state} cleanFilter={cleanFilter} />
                  )}
                  {item.type === "button" && (
                    <ButtonElement item={item} state={state} isSelected={isSelected} cleanFilter={cleanFilter} store={store} section={section} isFormVisible={isFormVisible} handleSubmitForm={handleSubmitForm} handleItemAction={handleItemAction} />
                  )}
                  {item.type === "link" && (
                    <LinkElement item={item} store={store} state={state} isSelected={isSelected} cleanFilter={cleanFilter} updateItem={updateItem} activePageId={activePageId} section={section} isFormVisible={isFormVisible} handleSubmitForm={handleSubmitForm} handleItemAction={handleItemAction} handleDoubleClick={handleDoubleClick} />
                  )}
                  {item.type === "input" && (
                    <InputElement item={item} state={state} cleanFilter={cleanFilter} />
                  )}
                </div>
              </React.Fragment>
            );
          ...
          {validTargets.length > 0 && !state.isPreviewMode && !isSelecting && (
            <Moveable
              ...
              snapHorizontal={true}
              snapCenter={true}
              // FIXED: proper canvas boundary — elements CANNOT exit the 1200px canvas
              bounds={{
                left: 0,
                top: 0,
                right: CANVAS_WIDTH,
              ...
              horizontalGuidelines={[canvasHeight / 2]}
              ...
              onDrag={({ target, left, top }) => {
                // Moveable already respects `bounds` from props — just apply the values
              ...
              onResize={({ target, width, height, drag }) => {
                // Moveable respects bounds — just apply computed values
          ...
          {!state.isPreviewMode && (
            <Selecto
              ...
              onDragStart={(e) => {
                ...
                if (t.closest(".canvas-element")) return;
                const rect = sectionRef.current?.getBoundingClientRect();
                if (!rect) return;
                e.datas.offset = [rect.left + window.scrollX, rect.top + window.scrollY];
              }}
              onSelect={(e) => {
      ...
      {showSuccessModal && (
        <div style={modalStyles.overlay} className="modal-content-box">
          <div style={modalStyles.content}>
            ...
            <button style={modalStyles.btn} onClick={() => setShowSuccessModal(false)}>ok</button>
          </div>
        </div>
      )}
      ...
      {showPopupSuccessModal && (
        <div style={modalStyles.overlay} className="modal-content-box">
          <div style={modalStyles.content}>
            ...
            <h2 style={{ margin: "10px 0", color: "#1e293b", fontSize: "20px" }}>Successful!</h2>
            <p style={{ color: "#64748b", fontSize: "14px", marginBottom: "20px" }}>
              Your data has been successfully saved. We look forward to your wonderful visit!
            </p>
            <button style={modalStyles.btn} onClick={() => setShowPopupSuccessModal(false)}>Awesome</button>
          </div>
        </div>
      )}
    </div>
